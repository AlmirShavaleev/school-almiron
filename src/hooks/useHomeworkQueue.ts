...
import { loadPendingQueueItems, type QueueBucket, type QueueItem } from '@/lib/pendingQueue'
...
export type { QueueBucket, QueueItem }
...
export function useHomeworkQueue(mode: QueueMode = 'pending', checkedLimit = DEFAULT_CHECKED_LIMIT) {
  ...
  useEffect(() => {
    ...
    async function load() {
      if (mode === 'pending') {
        const list = await loadPendingQueueItems(supabase as any, profile)
