...
export function useVariants(filters?: {
  ...
  const load = useCallback(async () => {
    ...
    let q = db
      ...
      // Одна таблица обслуживает две разные сущности: самостоятельный вариант
      // (`topic_id` пуст) и набор задач к уроку — вариант-носитель темы (§164,
      // `topic_id` заполнен). «Тесты» по определению про первую, поэтому
      // носители отсекаются здесь, в единственном запросе списка, а не
      // фильтром в каждом компоненте. Условие не лишнее: без него в «Физике
      // ЕГЭ» сверху ложились сотни строк «Задачи к уроку «…»», собранный
      // вариант тонул под ними, счётчик на карточке экзамена считал их же, а
      // корзина строки списка молча сносила задачи к уроку у целого класса.
      .is('topic_id', null)
