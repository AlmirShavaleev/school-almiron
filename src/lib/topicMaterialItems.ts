...
/**
 * Рубрики, скрытые с темы.
 *
 * Сейчас пусто. 12.08 сюда попала `test`: владелец отказался выдавать тесты на
 * темах. 12.09 он вернулся к замыслу в другом виде — «задачи к уроку, как в
 * Stepik» (§162), — и рубрика снова показывается, уже под именем «Задачи».
 *
 * Механизм остаётся переключателем на будущее: скрыть рубрику — дописать её
 * сюда, больше нигде править не нужно. Из `TOPIC_SECTION_ORDER` рубрики при
 * этом не убираются: перечень един (§100), и сторож «группы плюс остаток
 * покрывают весь перечень» обязан видеть их все.
 */
export const TOPIC_SECTIONS_HIDDEN: readonly TopicSection[] = [] as const
 ...
 *
 * Состав — как назвал владелец 10.08, плюс четвёртая группа от 12.09.
 *  - `theory` («Теория» как раздел материалов) — в группу «Теория», к видео и
 *    конспекту: это буквально её название;
 *  - `test` — до §162 НИ В ОДНОЙ группе: владелец назвал ровно три, и
 *    выдумывать четвёртую подпись было не нашим делом. В §162 он назвал её
 *    сам — «Задачи», — и решил, что решённые задачи считаются в «тема
 *    пройдена». Поэтому группа появилась.
 *
...
export interface TopicSectionGroup {
  key: 'theory' | 'lesson' | 'homework' | 'tasks'
  label: string
...
export interface TopicSectionRow {
  /** null — остаток, не попавший ни в одну группу. Сейчас все рубрики
   *  разложены по группам, ряд остаётся страховкой от забытой рубрики. */
