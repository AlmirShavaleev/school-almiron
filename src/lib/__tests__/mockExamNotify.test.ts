...
describe('Telegram-карточка mock_exam_result', () => {
  ...
  it('§223: с таблицей перевода — итог, под ним первичный, под ним части', () => {
    ...
    expect(text).toBe(
      ...
      'Результат: 72 балла из 100',
    )
  })

  it('строка «Результат» — со склонением и только при таблице перевода', () => {
    const r = (score: number) => buildMockExamResultTelegramMessage({ ...base, score, max_score: 100, primary_score: 5 }).text
    expect(r(81)).toContain('Результат: 81 балл из 100')
    expect(r(88)).toContain('Результат: 88 баллов из 100')
    expect(r(11)).toContain('Результат: 11 баллов из 100')
    expect(r(94)).toContain('Результат: 94 балла из 100')
    // Без таблицы (итог = первичный) строки нет — был бы повтор заголовка.
    expect(buildMockExamResultTelegramMessage(base).text).not.toContain('Результат:')
  })
