/**
 * Полный импорт каталога ФИЗИКИ ЕГЭ — КОПИЯ под отдельный пакет.
 *
 * Отличие от scripts/import-physics-catalog-full.mjs ровно одно:
 * FULL_DIR и IMAGES_DIR берутся из переменных окружения
 * CATALOG_FULL_DIR / CATALOG_IMAGES_DIR (значения по умолчанию — прежние).
 * Общий скрипт не трогаем: он зашит на школковский дамп.
 *
 * Прогон пакета в dry-run (ничего не пишется):
 *   CATALOG_FULL_DIR=.../out CATALOG_IMAGES_DIR=.../out/assets \
 *     node scripts/import-physics-catalog-package.mjs --dry-run
 *
...
const EXAM_TYPE  = 'ЕГЭ'
const FULL_DIR   = process.env.CATALOG_FULL_DIR
  ?? 'D:/школково спарсенные файлы/shkolkovo_physics_catalog/outputs/normalized_catalog'
const IMAGES_DIR = process.env.CATALOG_IMAGES_DIR
  ?? 'D:/школково спарсенные файлы/shkolkovo_physics_catalog/outputs/shkolkovo_physics_images'
