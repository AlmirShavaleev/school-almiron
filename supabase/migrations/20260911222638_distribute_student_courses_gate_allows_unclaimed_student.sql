-- §150. Ворота teacher_students были невыполнимы для НОВОГО ученика: связь
-- заводит только триггер §144, при зачислении в группу -- а этот гейт стоит
-- ДО зачисления. Замкнутый круг: чтобы зачислить, нужна связь, чтобы
-- появилась связь, нужно зачислить. Живой пример: Алексей Петров, 0 групп,
-- 0 связей -- distribute_student_courses падал FORBIDDEN: not your student
-- на первой же попытке.
--
-- Новое правило: своя активная связь -- пускаем как раньше; связи ни с кем
-- нет -- пускаем, ничей ученик не может быть «чужим» (связь заведёт §144 при
-- самом зачислении); связь с ДРУГИМ преподавателем -- отказ остаётся, ворота
-- ставились именно против этого; админ/владелец -- пускаем всегда.
--
-- pg_advisory_xact_lock на student_id: старый `FOR UPDATE` лочил только
-- строку, совпадающую по (teacher_id, student_id, status='active') -- если
-- строки ещё нет вовсе (ничей ученик), блокировать было нечего, и два
-- преподавателя могли бы забрать одного ученика одновременно.
--
-- Отказ -- читаемым текстом с HINT, как в §136/§145, а не голым FORBIDDEN.

create or replace function public.distribute_student_courses(p_student_id uuid, p_assignments jsonb, p_request_id uuid DEFAULT NULL::uuid)
 returns jsonb
 language plpgsql
 security definer
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtext('distribute_student_courses:' || p_student_id::text));
  ...
  IF NOT (
    ...
    OR v_claimed_by = v_teacher_id
  ) THEN
    RAISE EXCEPTION 'Этот ученик уже закреплён за другим преподавателем.'
      USING ERRCODE = 'P0001', HINT = 'STUDENT_CLAIMED_BY_OTHER_TEACHER';
  END IF;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_assignments) LOOP
    ...
    ELSIF v_mode IN ('individual', 'new_group') AND EXISTS (
      SELECT 1 FROM public.groups g WHERE g.course_id = v_course_id
    ) THEN
      -- §61/§64: у курса уже есть группа -- вторую завести нельзя
      -- (groups_one_per_course), а голая ошибка уникальности до пользователя
      -- доходить не должна.
      RAISE EXCEPTION 'У курса «%» уже есть группа — добавьте ученика в неё.',
        (SELECT c.title FROM public.courses c WHERE c.id = v_course_id)
        USING ERRCODE = 'P0001', HINT = 'COURSE_ALREADY_HAS_GROUP';
