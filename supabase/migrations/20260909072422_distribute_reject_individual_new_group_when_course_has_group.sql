-- §61/§64 сделали «один курс = одна группа» законом схемы: у КАЖДОГО курса
-- теперь ровно одна группа с рождения (триггер courses_ensure_group_trg).
-- Оба RPC распределения (distribute_student_courses, distribute_join_request)
-- этого не знали: режимы individual/new_group при отсутствии активного
-- членства ученика всё ещё пытались вставить ВТОРУЮ группу на курс и
-- напарывались на unique-индекс groups_one_per_course голым
-- "duplicate key value violates unique constraint" — 409 без объяснения,
-- интерфейс показывал «Не удалось распределить ученика».
--
-- Правим на границе, до попытки INSERT: если у курса уже есть группа (а она
-- есть всегда, кроме исторической страховки для курсов старше триггера),
-- ошибка человекочитаемая и с HINT-кодом для клиента — тот же приём, что и
-- в group_students_reject_template (§136).

create or replace function public.distribute_join_request(p_join_request_id uuid, p_assignments jsonb, p_request_id uuid DEFAULT NULL::uuid)
 returns jsonb
 language plpgsql
 security definer
