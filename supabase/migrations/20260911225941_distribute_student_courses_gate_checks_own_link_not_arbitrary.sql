-- board/006. Уникальность в teacher_students стоит на паре (teacher_id,
-- student_id), а не на student_id -- ученик МОЖЕТ быть закреплён за двумя
-- преподавателями (два предмета, разные преподаватели, общий ученик -- не
-- край, а нормальный случай школы). `LIMIT 1` из §150 выбирал ПРОИЗВОЛЬНУЮ
-- строку: если она оказывалась чужой, а моя связь была рядом, отказ
-- приходил на СВОЁМ ЖЕ ученике.
--
-- Чиним: проверяем факт МОЕЙ связи через EXISTS, а не личность случайной
-- строки. Порядок: своя связь есть -> пускаем; иначе если есть хоть
-- одна активная связь (значит, чужая) -> отказ; связей нет вовсе -> пускаем.
...
DECLARE
  ...
  v_has_own_link boolean;
  v_has_any_link boolean;
...
BEGIN
  ...
  SELECT
    EXISTS (SELECT 1 FROM public.teacher_students ts WHERE ts.student_id = p_student_id AND ts.status = 'active' AND ts.teacher_id = v_teacher_id),
    EXISTS (SELECT 1 FROM public.teacher_students ts WHERE ts.student_id = p_student_id AND ts.status = 'active')
  INTO v_has_own_link, v_has_any_link;

  IF NOT (
    public.is_admin_or_owner()
    OR v_has_own_link
    OR NOT v_has_any_link
  ) THEN
