-- Пустая сдача давала NULL вместо 0.
--
-- SUM(points_earned) по нулю строк ответов — это NULL, а не 0. Дальше NULL
-- уходил и в test_variant_student_assignments.score, и в payload карточки: у
-- ученика оказывался пустой балл при max_score = 10, а воркер, не увидев числа,
-- печатал обобщённое «Вариант проверен» без процента. Пустая сдача — это ноль
-- баллов, а не «без оценки»; умолчание здесь врёт.
--
-- Правка в обоих местах, где считается сумма: в идемпотентной ветке и в
-- основной. Заодно идемпотентная ветка получила ту же подстраховку по максимуму
-- (COALESCE на tvsa.max_score), что и основная: без неё повторный вызов по уже
-- проверенной работе мог вернуть максимум 0 и процент NULL, то есть ответить
-- хуже, чем записано в самой строке.
--
-- Бэкфилл не потребовался: живых строк со статусом submitted/completed/graded и
-- пустым баллом на проде нет (проверено перед правкой).
