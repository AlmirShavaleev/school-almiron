-- ============================================================
-- Homework v2 — split the shared 3/min attempt-action bucket into separate start/finalize
-- budgets. Audit: start A -> finalize A -> start B -> finalize B (two quick real submissions)
-- is 4 actions against one shared 'homework_attempt_action' bucket, which can trip the 3/min
-- cap for a normal (non-bot) student doing two things back to back. Splitting into
-- 'homework_attempt_start' and 'homework_attempt_finalize', each capped at 3/min, keeps the
-- same per-action anti-burst strength while letting a normal two-assignment burst through.
-- Hourly cap (60, set in 20260722200000) is untouched. New additive migration.
-- ============================================================
...
begin
  ...
  perform public._enforce_homework_rate_limit(v_student_id, 'homework_attempt_start', 3, 60);
...
begin
  ...
  perform public._enforce_homework_rate_limit(v_student_id, 'homework_attempt_finalize', 3, 60);
