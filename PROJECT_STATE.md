...
- [ ] ЧАТ А — Обкатка боем: полный сценарий на alminion.ru
...
- [ ] ЧАТ Б — Ученик: карточки прогресса читают легаси topic_materials
      (has_notes/has_theory в useStudentCourseProgram, LessonDetailPage) —
      перевести на topic_material_items.section + наличие ДЗ/теста;
      затем журнал: get_student_journal не знает topic_homework (score!)
      и topic_test_attempts — переписать (см. §5.4; только фронт+RPC-чтение,
      миграция читающей функции — согласовать с правилом выше).
- [ ] ЧАТ В — Хвосты инфраструктуры: R2 (env VITE_ASSETS_BASE_URL в Vercel +
      редеплой, §8), шаблон письма «смена email», чистка корня репо
      (+ .gitignore: _to_delete/, *.tar.gz, import-логи, скриншоты).
